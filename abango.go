...
func RunEndRequest(docroot string, params string, body string) string {
	...
	if err := GetXConfig(enddir); err == nil {
		...
		if XConfig["ApiType"] == "Kafka" {
			e.MyLog(enddir+"abango.log", "enddir="+enddir)
			e.Tp("B")
